...
EventUtil.addHandler(root,"click",function(event){
  ...
  if(!clickName){//未选中其他元素
    clickName=target;
    clickParent=target.parentNode;
    target.style.backgroundColor="#BECCD8";
  }else if(delName==target){//点击两次
...
EventUtil.addHandler(delBtn,"click",function(){
 // var len=delName.length;
  if(!clickName){
    alert("未选中删除元素");
  }else{
    clickParent.removeChild(clickName);
    clickName=null;
    clickParent=null;
  }
})
...
EventUtil.addHandler(addBtn,"click",function(){
   var addValue=document.getElementById("add-input").value;
  if(!addValue){
    alert("请输入插入节点内容");
  }
  else if(!clickName){
    ...
    var parentClassValue=clickName.className.replace(/[^0-9]/ig,""); 
    if(parseInt(parentClassValue)<3){
      node.className="div"+(parseInt(parentClassValue)+1)+" flexs";
    }else{
      node.className="div"+(parseInt(parentClassValue)+1);
    }
    clickName.appendChild(node);
    clickName.style.backgroundColor="white";
    clickName=null;
    clickParent=null;
